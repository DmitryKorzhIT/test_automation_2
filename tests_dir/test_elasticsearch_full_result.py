...
def test_1():
    ...
    file_name = elasticsearch_full_result.create_report_file(test_name='elasticsearch_full_result_error_404',
                                                             header=header)
    ...
    for search_query in search_queries:
        ...
        pages_links_list = elasticsearch_full_result.check_pages_search()  # return list of all the links in the search result.

        for page in pages_links_list:
            elasticsearch_full_result.get(page)
            if elasticsearch_full_result.is_404_error() == True:
                flag = False
                elasticsearch_full_result.check_pages_elasticsearch_report(file_name=file_name,
